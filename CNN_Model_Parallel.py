...
class CNNModel(object):
    ...
    def train(self,parallel=False,fold_index=0):
        self.build()
        cross_validation=10
        ...
        if parallel is False:
            ...
            print("Global step:",self.global_step.eval(session=self.sess))
            #record the steps f score
            self.train_f_score=[[]]*10
            #record the cross entropy each step during training
            iteration_error=[]
            for i in range(self.train_epoch):
            ...
            with open(self.pickle_file_path+'f_score_steps_'+pickle_file_pretrain_indicator+'_training_fold'+str(c+1)+'.pickle', 'wb') as f:
                # Pickle the 'data' dictionary using the highest protocol available.
                pickle.dump(self.train_f_score, f, pickle.HIGHEST_PROTOCOL)
...
def build_model(folder_name,fold_index):
    print(folder_name)
    ...
    print("Model "+str(fold_index))
    Model=CNNModel(fold_index,ckpt_file_path,tfrecords_file_path,tfrecords_file_path_pretrain,pickle_file_path,ckpt_file,folder_name,tfrecords_file_path_gang)
    Model.train(True,fold_index)
...
if __name__ == '__main__':
    ...
    for folder_name in ['CP_TW']:
        ...
        Parallel(n_jobs=num_cores)(delayed(build_model)(folder_name,ii) for ii in range(cv))
